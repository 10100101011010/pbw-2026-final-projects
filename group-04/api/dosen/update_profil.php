...
if (!empty($_FILES['foto_profil']['name'])) {
    ...
    $namaAman = 'dosen_' . $dosenId . '_' . time() . '.' . $ekstensi;
    ...
    $stmtFotoLama = $pdo->prepare('SELECT foto_profil FROM dosen WHERE id = ?');
    $stmtFotoLama->execute([$dosenId]);
...
try {
    if ($passwordBaru !== '') {
        ...
        $stmt = $pdo->prepare("UPDATE dosen SET nama=?, email=?, no_hp=?, password=?$kolomFoto WHERE id=?");
        $stmt->execute(array_merge([$nama, $email, $no_hp, $hash], $paramFoto, [$dosenId]));
    } else {
        $stmt = $pdo->prepare("UPDATE dosen SET nama=?, email=?, no_hp=?$kolomFoto WHERE id=?");
        $stmt->execute(array_merge([$nama, $email, $no_hp], $paramFoto, [$dosenId]));
    }
...
if (!empty($paramFoto)) {
    $_SESSION['dosen_foto'] = $paramFoto[0];
}
